...
| 015 | **Claude Code platform-capability tracking + adoption** — single-source the encoded platform surface (`platform.ts`, version-stamped) + widen the gate to the mid-2026 spec (`ConfigChange` + 8 more hook events; `defaultEnabled`/`userConfig`/`channels`/`lspServers`/`experimental` manifest fields; marketplace `defaultEnabled`/`relevance`), forge-generatable monitors + LSP, SchemaStore `$schema`, prose/KB catch-up, optional design→theme sink, and a recurring `platform-watch` governance meta-plugin | P1 (WP1–3) | S–M ×8 WPs | — | PLANNED (research notes: `notes/015-claude-code-platform-findings.md`) |
| 016 | **Epic: agentic demo studio** — `@objectcore/demo` engine (`deriveDemo` seam + the deterministic de-slop/evidence/live-safety/video gate + curated demo archetypes), the `demo-studio` plugin, and `@objectcore/demo-mcp`; makes "transparency over spectacle" a gate rather than a prose aspiration | P2 | L (P0–P6) | Stage 1 patterns + plan 012/014 shapes | **BUILT (P0–P6)** on branch `feat/016-demo-studio`. Engine + gate + judged layer + 2 archetypes + plugin (via `/forge`) + video track + MCP seam + dogfood `demos/objectcore/`. **Activation/delegation evals and the `eval:record` before/after still need a CI run with the API key** (hard rule #5). Research digest `notes/016-demo-studio-research.md`; forge gap report `notes/016-demo-studio-forge-gaps.md` (L1) |
